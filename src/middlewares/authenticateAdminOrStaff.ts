...
/**
 * Middleware that accepts both ADMIN and STAFF roles.
 * Used on endpoints that kitchen staff need access to (order list, status updates).
 */
        ...
        if (admin_accessToken) {
            ...
            if (user) {
                if (user.role === EUserRoles.ADMIN || user.role === EUserRoles.STAFF) {
                    ...
                    return httpError(next, new Error('Admin or Staff access required'), request, 403)
                }
            }
        }
